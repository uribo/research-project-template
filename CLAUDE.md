...
- **数値結果を直接左右するパッケージは版ピン留めターゲットで cue に載せる**（モデル推定・乱数・数値計算系に限定。整形系は載せない）。targets は既定でパッケージ版を cue に含めず、`tar_option_set(imports)` は本テンプレートのスタイルでは**機能しない** — 名前空間付き呼び出し（`pkg::fun()`）は自由シンボルとして追跡されず（`targets::tar_deps(coxme::coxme(x))` は `::` と `x` しか返さない。2026-08-26 実測）、survival / glmmTMB / emmeans などは名前空間単体で DAG 循環を起こす。代わりに `tar_target(model_pkg_versions, sapply(c(...), \(p) as.character(packageVersion(p))))` を置き、結果に直結するターゲットだけ `{model_pkg_versions; fit_model(...)}` のブレース参照で依存させる（雛形は `_targets.R` のコメント）。版だけ上がってコード同一でも再計算が走る粗さは受け入れる。CmdStan 等の R 外バックエンドも版をピンターゲットに含めれば同じ機構に載る。reproducibility sentinel と「lockfile 更新後の再ビルド規律」（「再現性」節）の代替ではなく補完
- **`tar_make()` が動いているかの確認は 2 系統でやる。** `tar_make()` は `callr` 経由で `R --slave` を起動するため `pgrep "Rscript"` では検出できない。`_targets/meta/process` に pid が書かれるが、**このレコードはプロセスが死んでも残る**ので pid の存在は生存の証拠にならない。`ps -p <pid>` で生存を確かめ、あわせて進捗が実際に増えているか（ログの mtime、`_targets/objects/` やパッケージのキャッシュのファイル数）を独立に見る。長いビルドは `nohup` で切り離して起動する（「Workflow」の「長時間のバックグラウンド実行と生存確認」）。Reason: 2026-09-01 に `pgrep` で見えないことを「殺された」と誤読し、2026-09-02 には残った pid レコードを「実行中」と誤読した
...
- **ローカルの入力パスが違う**: `list.files()` が誤ったパスに対して `character(0)` を返すと、読み込みは **0 行かつ 0 列**のデータフレームを作り、意味的検証は「キー列が全部無い」と報告する。エラーは列名を挙げるので**供給側のヘッダードリフトのように読めるが、実際の原因は上流のディレクトリ改名**。「列が無い」と言われたら、スキーマを疑う前に `dir.exists()` を疑う。`require_input_dir()` / `list_input_files()` を読み込みの手前に置く。ディレクトリが空という主張は `allow_empty = TRUE` で**呼び出し側に明示させる**（既定で滑り込ませない）
- **取得が失敗した**: ネットワークエラーと「照会したが該当なし」は、どちらも 0 件を手元に残す。`n = 0` として記録すると、**後から「存在しない証拠」として読まれる**。`new_fetch_result()` は 4 値 status（`ok` / `empty` / `absent` / `failed`）を強制し、成功時は `n` が数値・失敗時は `n` が `NA` でなければエラーにする。したがって失敗を 0 件として保存できない。`summarise_fetch_results()` は**失敗を分母から外し、その件数を併記**する（成功分のみで計算した率は母集団に対する率ではないので、除外件数を必ず一緒に運ぶ）
...
- `## YYYY-MM-DD の追記` / `の作業` など日付を冠した見出しで下へ堆積させない。上書きで更新する。`memory/` は git 管理なので、削った内容は `git log -p memory/project-status.md` で復元できる
