...
jarl は同節の手順で CLI を別途導入し、`jarl --version` が CI と同じ `jarl 0.6.0` を返すことを確認する。VS Code / Positron の推奨拡張を入れるだけでは CLI の導入・版固定は完了しない。テンプレート複製後、生成先のルートで `jarl check .` が通ることも確認する。
